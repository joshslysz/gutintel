...
class Top100ImporterFinal:
    ...
    async def insert_ingredient(self, db, row):
        ...
        try:
            # Parse aliases as array (not JSON string)
            aliases = []
            ...
            # Insert ingredient - aliases as array, dosage_info as JSON string
            ...
            ''',
                ...
                aliases,  # Pass as Python list, not JSON string
                json.dumps(dosage_info)  # Pass as JSON string
            )
...
async def main():
    if len(sys.argv) != 2:
        print("Usage: python import_top_100_final.py <csv_file>")
        print("Example: python import_top_100_final.py gut_health_ingredients.txt")
        sys.exit(1)
    ...
    print("🧬 GUTINTEL TOP 85 IMPORT (FINAL VERSION)")
    print("=" * 50)
    
    importer = Top100ImporterFinal()
